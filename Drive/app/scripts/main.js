 ...
 */
geotab.addin.autoBorderSwitcher = function () {
  'use strict';
  ...
  let switchUserRuleset = async function(){
    if (activeUser.hosRuleSet == availableRulesets[0]) {
      activeUser.hosRuleSet = availableRulesets[1];
    }
    else if (activeUser.hosRuleSet ==  availableRulesets[1]) {
      activeUser.hosRuleSet =  availableRulesets[0];
    }
    else {
      console.log('Cannot change ruleset');
    }
    console.log(activeUser);
    let setUserResult = await autoBorderSwitcher.databaseModule.setUser(api, activeUser);
    if(setUserResult){
      await getActiveUser();
    }
    else{
      switcherButton.disabled = true;
      document.getElementById('isStatus').innerHTML = 'Status: Failed to update Ruleset, please check back later!';
    }
  },
  ...
  return {
    ...
    //   populateRulesetInfo();
      ...
      // api.mobile.notification.notify("This is my first notification", "Notifcation 1").then(
      //   result => console.log(result) 
      // ).catch(
      //   error => console.log(error)
      // )
      // console.log("api.mobile exists!: ", api.mobile);
      // console.log(api.mobile.notify("This is my second notification", "Notifcation 2"));
      // console.log("api.mobile!: ", api.mobile.notification);
